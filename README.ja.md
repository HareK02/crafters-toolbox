...
| `ssh start/stop`                    | 協調作業用 SSH サーバーコンテナを起動・停止する                                                       |
| `ssh keys`                          | `.ssh/authorized_keys` を操作（`add` / `list` / `remove` / `path`）                                 |
| `components list`                   | `crtb.properties.yml` で定義されたコンポーネントの一覧表示                                           |
| `components update [names...]`      | ローカルのソースを使ってビルド・デプロイする（リモート取得は行わない）                               |
| `components pull [names...]`        | リモートからソースを取得し、ビルド・デプロイまで一括で行う（ローカルソースはスキップ）               |
| `components import [names...]`      | `./components/` の未登録コンポーネントを `crtb.properties.yml` に登録する                            |
| `terminal [game\|ssh\|monitor]`     | 稼働中コンテナのコンソールにアタッチする                                                             |
| `init [dir] [--no-prompt]`          | 新しいプロジェクトを初期化する                                                                       |
| `setup`                             | `server.jar` をダウンロードする                                                                      |

詳細は [`docs/ja/components.md`](docs/ja/components.md) を参照してください。

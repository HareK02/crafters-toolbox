...
function displayPathInstructions(installDir: string): void {
  ...
  console.log();
  console.log(
    `${colors.blue}Note:${colors.reset} ${installDir} is not in your PATH.`,
  );
  ...
  if (Deno.build.os === "windows") {
    ...
    console.log(
      `  export PATH="${installDir}:$PATH"  # bash/zsh (.bashrc, .zshrc)`,
    );
    console.log(
      `  set -Ux fish_user_paths ${installDir} $fish_user_paths  # fish (config.fish)`,
    );
  }
  console.log();
}
...
async function main() {
  ...
  try {
    ...
    const installDir = Deno.env.get("INSTALL_DIR_OVERRIDE") ||
      DEFAULT_INSTALL_DIR;
